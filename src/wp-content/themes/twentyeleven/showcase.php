...
if (!empty($sticky)) {
    $featured_args = array('post__in' => $sticky, 'post_status' => 'publish', 'posts_per_page' => 10, 'no_found_rows' => true);
    ...
    if ($featured->have_posts()) {
        ...
        while ($featured->have_posts()) {
            ...
            ?>
					...
					<section class="featured-post <?php 
            echo esc_attr($feature_class);
            ?>
" id="featured-post-<?php 
            echo esc_attr($counter_slider);
            ?>
">
            ...
            if (has_post_thumbnail()) {
                ...
                ?>
" title="<?php 
                echo esc_attr(sprintf(__('Permalink to %s', 'twentyeleven'), the_title_attribute('echo=0')));
                ?>
" rel="bookmark"><?php 
        ...
        if ($featured->post_count > 1) {
            ...
            while ($featured->have_posts()) {
                ...
                ?>
						<li><a href="#featured-post-<?php 
                echo esc_attr($counter_slider);
                ?>
" title="<?php 
                echo esc_attr(sprintf(__('Featuring: %s', 'twentyeleven'), the_title_attribute('echo=0')));
                ?>
"<?php 
                echo $class;
                ?>
...
$recent_args = array('order' => 'DESC', 'post__not_in' => get_option('sticky_posts'), 'tax_query' => array(array('taxonomy' => 'post_format', 'terms' => array('post-format-aside', 'post-format-link', 'post-format-quote', 'post-format-status'), 'field' => 'slug', 'operator' => 'NOT IN')), 'no_found_rows' => true);
