...
if ($wp_query->max_num_pages > 1) {
    ...
    next_posts_link(__('<span class="meta-nav">&larr;</span> Older posts', 'twentyten'));
    ?>
</div>
		<div class="nav-next"><?php 
    previous_posts_link(__('Newer posts <span class="meta-nav">&rarr;</span>', 'twentyten'));
    ?>
</div>
	</div><!-- #nav-above -->
<?php 
...
if (!have_posts()) {
    ...
    ?>
		...
		<div class="entry-content">
			<p><?php 
    _e('Apologies, but no results were found for the requested archive. Perhaps searching will help find a related post.', 'twentyten');
    ?>
</p>
			<?php 
    get_search_form();
            ...
            if ($images) {
                ...
                ?>
						<div class="gallery-thumb">
							<a class="size-thumbnail" href="<?php 
                the_permalink();
                ?>
"><?php 
                ...
                ?>
						...
						<p><em><?php 
                printf(_n('This gallery contains <a %1$s>%2$s photo</a>.', 'This gallery contains <a %1$s>%2$s photos</a>.', $total_images, 'twentyten'), 'href="' . esc_url(get_permalink()) . '" title="' . esc_attr(sprintf(__('Permalink to %s', 'twentyten'), the_title_attribute('echo=0'))) . '" rel="bookmark"', number_format_i18n($total_images));
                ?>
</em></p>
				<?php 
            }
            // end twentyten_get_gallery_images() check
            ?>
						<?php 
            the_excerpt();
        }
...
"><?php 
			...
			<?php 
        } elseif ($gallery = get_term_by('slug', _x('gallery', 'gallery category slug', 'twentyten'), 'category') && in_category($gallery->term_id)) {
            ?>
				<a href="<?php 
            echo esc_url(get_category_link($gallery));
            ?>
" title="<?php 
            esc_attr_e('View posts in the Gallery category', 'twentyten');
            ?>
"><?php 
            _e('More Galleries', 'twentyten');
            ?>
