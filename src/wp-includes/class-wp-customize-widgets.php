...
class WP_Customize_Widgets
{
    ...
    public function output_widget_control_templates()
    {
        ...
        ?>
				...
				<input type="search" id="widgets-search" placeholder="<?php 
...
" />
        ...
        foreach ($this->get_available_widgets() as $available_widget) {
            ...
            echo esc_attr($available_widget['id']);
            ?>
" data-widget-id="<?php 
            echo esc_attr($available_widget['id']);
            ?>
" class="widget-tpl <?php 
            echo esc_attr($available_widget['id']);
            ?>
" tabindex="0">
					<?php 
...
;
    ...
    public function wp_ajax_update_widget()
    {
        ...
        $updated_widget = $this->call_widget_update($widget_id);
        // => {instance,form}
    ...
    public function customize_dynamic_partial_args($partial_args, $partial_id)
    {
        ...
        if (preg_match('/^widget\\[(?P<widget_id>.+)\\]$/', $partial_id, $matches)) {
            ...
            $partial_args = array_merge($partial_args, array('type' => 'widget', 'render_callback' => array($this, 'render_widget_partial'), 'container_inclusive' => true, 'settings' => array($this->get_setting_id($matches['widget_id'])), 'capability' => 'edit_theme_options'));
        }
        return $partial_args;
    }
    /**
    ...
    {
        $sidebar_args = array_merge(array('before_widget' => '', 'after_widget' => ''), $params[0]);
    ...
    public function filter_wp_kses_allowed_data_attributes($allowed_html)
    {
        foreach (array_keys($this->before_widget_tags_seen) as $tag_name) {
            ...
            $allowed_html[$tag_name] = array_merge($allowed_html[$tag_name], array_fill_keys(array('data-customize-partial-id', 'data-customize-partial-type', 'data-customize-partial-placement-context', 'data-customize-partial-widget-id', 'data-customize-partial-options'), true));
        }
        return $allowed_html;
    }
    /**
