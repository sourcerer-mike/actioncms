...
class WP_Admin_Bar
{
    ...
    protected final function _bind()
    {
        ...
        $this->add_node(array('id' => 'root', 'group' => false));
        ...
        foreach ($this->_get_nodes() as $node) {
            ...
            if ($parent->type == 'item' && $node->type == 'item') {
                ...
                if (!$default) {
                    ...
                    $this->_set_node(array('id' => $default_id, 'parent' => $parent->id, 'type' => 'group', 'children' => array(), 'meta' => array('class' => $group_class), 'title' => false, 'href' => false));
                    $default = $this->_get_node($default_id);
                ...
                if (!$container) {
                    // Use _set_node because add_node can be overloaded.
                    // Make sure to specify default settings for all properties.
                    $this->_set_node(array('id' => $container_id, 'type' => 'container', 'children' => array($parent), 'parent' => false, 'title' => false, 'href' => false, 'meta' => array()));
                    $container = $this->_get_node($container_id);
...
"><?php 
    ...
    protected final function _render_container($node)
    {
        ...
        ?>
<div id="<?php 
        echo esc_attr('wp-admin-bar-' . $node->id);
        ?>
...
"<?php 
            ...
            if (!empty($node->meta['target'])) {
                ?>
 target="<?php 
                echo esc_attr($node->meta['target']);
                ?>
"<?php 
            }
