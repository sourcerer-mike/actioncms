...
class getid3_lib
{
    public static function PrintHexBytes($string, $hex = true, $spaces = true, $htmlencoding = 'UTF-8')
    {
        ...
        for ($i = 0; $i < strlen($string); $i++) {
            if ($hex) {
                ...
                $returnstring .= ' ' . (preg_match("#[ -~]#", $string[$i]) ? $string[$i] : 'Â¤');
            }
            if ($spaces) {
                $returnstring .= ' ';
            }
    ...
    public static function BigEndian2String($number, $minbytes = 1, $synchsafe = false, $signed = false)
    {
        ...
        return str_pad($intstring, $minbytes, " ", STR_PAD_LEFT);
    }
    ...
    {
        return self::BigEndian2Int(substr($rawdata, 0, 1)) + (double) (self::BigEndian2Int(substr($rawdata, 1, 1)) / pow(2, 8));
    }
    public static function FixedPoint16_16($rawdata)
    {
        return self::BigEndian2Int(substr($rawdata, 0, 2)) + (double) (self::BigEndian2Int(substr($rawdata, 2, 2)) / pow(2, 16));
    }
    public static function FixedPoint2_30($rawdata)
    {
        $binarystring = self::BigEndian2Bin($rawdata);
        return self::Bin2Dec(substr($binarystring, 0, 2)) + (double) (self::Bin2Dec(substr($binarystring, 2, 30)) / pow(2, 30));
    }
    ...
    public static function iconv_fallback_utf8_utf16be($string, $bom = false)
    {
        ...
        while ($offset < $stringlength) {
            ...
            if ($charval !== false) {
                $newcharstring .= $charval < 65536 ? self::BigEndian2String($charval, 2) : " " . '?';
            }
        }
        return $newcharstring;
    }
    ...
    public static function iconv_fallback_utf8_utf16le($string, $bom = false)
    {
        ...
        while ($offset < $stringlength) {
            ...
            if ($charval !== false) {
                $newcharstring .= $charval < 65536 ? self::LittleEndian2String($charval, 2) : '?' . " ";
            }
        }
        return $newcharstring;
    }
    ...
    public static function iconv_fallback($in_charset, $out_charset, $string)
    {
        ...
        if (function_exists('iconv')) {
            if ($converted_string = @iconv($in_charset, $out_charset . '//TRANSLIT', $string)) {
                switch ($out_charset) {
                    case 'ISO-8859-1':
                        $converted_string = rtrim($converted_string, " ");
                        break;
                }
