...
;
    ...
    public function prepare_setting_validity_for_js($validity)
    {
        if (is_wp_error($validity)) {
            ...
            foreach ($validity->errors as $error_code => $error_messages) {
                ...
                $error_data = array_merge($error_data, array('from_server' => true));
                $notification[$error_code] = array('message' => join(' ', $error_messages), 'data' => $error_data);
            }
            return $notification;
        } else {
            return true;
    ...
    public function save()
    {
        ...
        if ($invalid_setting_count > 0) {
            $response = array('setting_validities' => $exported_setting_validities, 'message' => sprintf(_n('There is %s invalid setting.', 'There are %s invalid settings.', $invalid_setting_count), number_format_i18n($invalid_setting_count)));
        ...
        $data = array('setting_validities' => $exported_setting_validities);
        /**
    ...
    public function customize_pane_settings()
    {
        ...
        $settings = array('theme' => array('stylesheet' => $this->get_stylesheet(), 'active' => $this->is_theme_active()), 'url' => array('preview' => esc_url_raw($this->get_preview_url()), 'parent' => esc_url_raw(admin_url()), 'activated' => esc_url_raw(home_url('/')), 'ajax' => esc_url_raw(admin_url('admin-ajax.php', 'relative')), 'allowed' => array_map('esc_url_raw', $allowed_urls), 'isCrossDomain' => $cross_domain, 'home' => esc_url_raw(home_url('/')), 'login' => esc_url_raw($login_url)), 'browser' => array('mobile' => wp_is_mobile(), 'ios' => $this->is_ios()), 'panels' => array(), 'sections' => array(), 'nonce' => $this->get_nonces(), 'autofocus' => $this->get_autofocus(), 'documentTitleTmpl' => $this->get_document_title_template(), 'previewableDevices' => $this->get_previewable_devices());
