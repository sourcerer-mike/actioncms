...
function _split_str_by_whitespace($string, $goal)
{
    ...
    while ($goal < strlen($string_nullspace)) {
        $pos = strrpos(substr($string_nullspace, 0, $goal + 1), " ");
        if (false === $pos) {
            $pos = strpos($string_nullspace, " ", $goal + 1);
            if (false === $pos) {
                break;
            }
        }
...
function wp_sprintf_l($pattern, $args)
{
     ...
     */
    $l = apply_filters('wp_sprintf_l', array('between' => sprintf(__('%s, %s'), '', ''), 'between_last_two' => sprintf(__('%s, and %s'), '', ''), 'between_only_two' => sprintf(__('%s and %s'), '', '')));
    $args = (array) $args;
...
{
    global $wp_version;
    $settings = array('baseUrl' => apply_filters('emoji_url', 'https://s.w.org/images/core/emoji/2/72x72/'), 'ext' => apply_filters('emoji_ext', '.png'), 'svgUrl' => apply_filters('emoji_svg_url', 'https://s.w.org/images/core/emoji/2/svg/'), 'svgExt' => apply_filters('emoji_svg_ext', '.svg'));
    $version = 'ver=' . $wp_version;
    if (SCRIPT_DEBUG) {
        $settings['source'] = array('wpemoji' => apply_filters('script_loader_src', includes_url("js/wp-emoji.js?{$version}"), 'wpemoji'), 'twemoji' => apply_filters('script_loader_src', includes_url("js/twemoji.js?{$version}"), 'twemoji'));
        ?>
...
;
        ...
        $settings['source'] = array('concatemoji' => apply_filters('script_loader_src', includes_url("js/wp-emoji-release.min.js?{$version}"), 'concatemoji'));
        /*
