 ...
 <div id="message" class="updated"><p><?php 
	...
	<?php 
            if (wp_verify_nonce($_GET['_error_nonce'], 'plugin-activation-error_' . $file)) {
                $iframe_url = add_query_arg(array('action' => 'error_scrape', 'plugin' => urlencode($file), '_wpnonce' => urlencode($_GET['_error_nonce'])), admin_url('plugins.php'));
                ?>
	<iframe style="border:0" width="100%" height="70px" src="<?php 
                echo esc_url($iframe_url);
                ?>
"></iframe>
	<?php 
            }
...
<?php 
        foreach ($plugin_files as $plugin_file) {
            ...
            ?>
		<li<?php 
            echo $file == $plugin_file ? ' class="highlight"' : '';
            ?>
><a href="plugin-editor.php?file=<?php 
            echo urlencode($plugin_file);
            ?>
&amp;plugin=<?php 
            echo urlencode($plugin);
            ?>
"><?php 
            echo $plugin_file;
            ?>
...
<form name="template" id="template" action="plugin-editor.php" method="post">
	<?php 
        ...
        ?>
		<div><textarea cols="70" rows="25" name="newcontent" id="newcontent" aria-describedby="newcontent-description"><?php 
        echo $content;
        ?>
</textarea>
...
" />
 ...
 " onclick="if ( '' != jQuery('#docs-list').val() ) { window.open( 'https://api.wordpress.org/core/handbook/1.0/?function=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=<?php 
            echo urlencode(get_locale());
            ?>
&amp;version=<?php 
            echo urlencode($wp_version);
            ?>
&amp;redirect=true'); }" /></div>
		<?php 
        }
