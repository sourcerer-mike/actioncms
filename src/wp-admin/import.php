...
<table class="widefat importers striped">
    ...
    foreach ($importers as $importer_id => $data) {
        ...
        if (isset($data['install'])) {
            ...
            if (file_exists(WP_PLUGIN_DIR . '/' . $plugin_slug)) {
                ...
                if (!empty($plugins)) {
                    ...
                    $action = sprintf('<a href="%s" aria-label="%s">%s</a>', esc_url($url), esc_attr(sprintf(__('Run %s'), $data[0])), __('Run Importer'));
            ...
            if (empty($action)) {
                if (is_main_site()) {
                    ...
                    $action = sprintf('<a href="%1$s" class="install-now" data-slug="%2$s" data-name="%3$s" aria-label="%4$s">%5$s</a>', esc_url($url), esc_attr($plugin_slug), esc_attr($data[0]), esc_attr(sprintf(__('Install %s'), $data[0])), __('Install Now'));
                } else {
                    $action = sprintf(__('This importer is not installed. Please install importers from <a href="%s">the main site</a>.'), get_admin_url(get_current_network_id(), 'import.php'));
                }
            }
        } else {
            $url = add_query_arg(array('import' => $importer_id), self_admin_url('admin.php'));
            $action = sprintf('<a href="%1$s" aria-label="%2$s">%3$s</a>', esc_url($url), esc_attr(sprintf(__('Run %s'), $data[0])), __('Run Importer'));
            $is_plugin_installed = true;
        }
        if (!$is_plugin_installed && is_main_site()) {
            ...
            $action .= sprintf(' | <a href="%1$s" class="thickbox open-plugin-details-modal" aria-label="%2$s">%3$s</a>', esc_url($url), esc_attr(sprintf(__('More information about %s'), $data[0])), __('Details'));
        }
