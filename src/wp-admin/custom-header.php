...
<br />
	<?php 
            if (!current_theme_supports('custom-header', 'flex-height') && !current_theme_supports('custom-header', 'flex-width')) {
                ...
                if (!current_theme_supports('custom-header', 'flex-width')) {
                    printf(__('Images should be at least %s wide.') . ' ', sprintf('<strong>' . __('%d pixels') . '</strong>', get_theme_support('custom-header', 'width')));
                }
            } elseif (current_theme_supports('custom-header', 'flex-width')) {
                if (!current_theme_supports('custom-header', 'flex-height')) {
                    printf(__('Images should be at least %s tall.') . ' ', sprintf('<strong>' . __('%d pixels') . '</strong>', get_theme_support('custom-header', 'height')));
                }
            }
            if (current_theme_supports('custom-header', 'flex-height') || current_theme_supports('custom-header', 'flex-width')) {
                if (current_theme_supports('custom-header', 'width')) {
                    printf(__('Suggested width is %s.') . ' ', sprintf('<strong>' . __('%d pixels') . '</strong>', get_theme_support('custom-header', 'width')));
                }
                if (current_theme_supports('custom-header', 'height')) {
                    printf(__('Suggested height is %s.') . ' ', sprintf('<strong>' . __('%d pixels') . '</strong>', get_theme_support('custom-header', 'height')));
                }
            }
            ?>
</p>
...
<?php 
    ...
    public function step_2()
    {
        ...
        if (!current_theme_supports('custom-header', 'uploads')) {
            wp_die('<h1>' . __('Cheatin&#8217; uh?') . '</h1>' . '<p>' . __('The current theme does not support uploading a custom header image.') . '</p>', 403);
        }
...
" />
    ...
    public function step_3()
    {
        ...
        if (!empty($_POST['skip-cropping']) && !(current_theme_supports('custom-header', 'flex-height') || current_theme_supports('custom-header', 'flex-width'))) {
            wp_die('<h1>' . __('Cheatin&#8217; uh?') . '</h1>' . '<p>' . __('The current theme does not support a flexible sized header image.') . '</p>', 403);
        }
        if ($_POST['oitar'] > 1) {
    ...
    public final function create_attachment_object($cropped, $parent_attachment_id)
    {
        ...
        $object = array('ID' => $parent_attachment_id, 'post_title' => basename($cropped), 'post_mime_type' => $image_type, 'guid' => $url, 'context' => 'custom-header');
        return $object;
    }
    /**
