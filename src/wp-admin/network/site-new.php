...
if (isset($_REQUEST['action']) && 'add-site' == $_REQUEST['action']) {
    ...
    if (!is_wp_error($id)) {
        ...
        wp_mail(get_site_option('admin_email'), sprintf(__('[%s] New Site Created'), $current_site->site_name), sprintf(__('New site created by %1$s
...
if (is_subdomain_install()) {
    ...
    echo preg_replace('|^www\\.|', '', $current_site->domain);
    ?>
</span>
			<?php 
} else {
