...
switch ($wp_list_table->current_action()) {
    case 'add-tag':
        ...
        if (!current_user_can($tax->cap->edit_terms)) {
            wp_die('<h1>' . __('Cheatin&#8217; uh?') . '</h1>' . '<p>' . __('Sorry, you are not allowed to add this item.') . '</p>', 403);
        }
...
<p>
    ...
    printf(__('Deleting a category does not delete the posts in that category. Instead, posts that were only assigned to the deleted category are set to the category %s.'), '<strong>' . apply_filters('the_category', get_cat_name(get_option('default_category'))) . '</strong>');
    ?>
</p>
<?php 
