...
if (!current_user_can('switch_themes') && !current_user_can('edit_theme_options')) {
    wp_die('<h1>' . __('Cheatin&#8217; uh?') . '</h1>' . '<p>' . __('Sorry, you are not allowed to edit theme options on this site.') . '</p>', 403);
}
if (current_user_can('switch_themes') && isset($_GET['action'])) {
    if ('activate' == $_GET['action']) {
        ...
        if (!$theme->exists() || !$theme->is_allowed()) {
            wp_die('<h1>' . __('Cheatin&#8217; uh?') . '</h1>' . '<p>' . __('The requested theme does not exist.') . '</p>', 403);
        }
        ...
        if (!current_user_can('delete_themes')) {
            wp_die('<h1>' . __('Cheatin&#8217; uh?') . '</h1>' . '<p>' . __('Sorry, you are not allowed to delete this item.') . '</p>', 403);
        }
        if (!$theme->exists()) {
            wp_die('<h1>' . __('Cheatin&#8217; uh?') . '</h1>' . '<p>' . __('The requested theme does not exist.') . '</p>', 403);
        }
...
if (!is_multisite() && current_user_can('install_themes')) {
    ...
    ?>
" class="hide-if-no-js page-title-action"><?php 
...
<div class="theme<?php 
    ...
    ?>
" tabindex="0" aria-describedby="<?php 
...
"><?php 
	...
	<?php 
        ...
        ?>
		<a class="button button-secondary activate" href="<?php 
        echo $theme['actions']['activate'];
        ?>
" aria-label="<?php 
...
<table>
    ...
    foreach ($broken_themes as $broken_theme) {
		...
		<tr>
			<td><?php 
        echo $broken_theme->get('Name') ? $broken_theme->display('Name') : $broken_theme->get_stylesheet();
        ?>
</td>
			<td><?php 
        echo $broken_theme->errors()->get_error_message();
        ?>
</td>
			<?php 
        if ($can_delete) {
...
$aria_label = sprintf(_x('Activate %s', 'theme'), '{{ data.name }}');
?>
			<a class="button button-secondary activate" href="{{{ data.actions.activate }}}" aria-label="<?php 
echo $aria_label;
?>
"><?php 
_e('Activate');
...
?>
				<# if ( data.actions.activate ) { #>
					<a href="{{{ data.actions.activate }}}" class="button button-secondary activate" aria-label="<?php 
echo $aria_label;
?>
"><?php 
_e('Activate');
...
?>
			...
			<# if ( ! data.active && data.actions['delete'] ) { #>
				<a href="{{{ data.actions['delete'] }}}" class="button button-secondary delete-theme"><?php 
_e('Delete');
?>
</a>
			<# } #>
