...
class WP_Press_This
{
    ...
    {
        return array('redirInParent' => apply_filters('press_this_redirect_in_parent', false));
    }
    /**
    ...
    public function add_category()
    {
        ...
        foreach ($added as $new_cat_id) {
            ...
            $data[] = array('term_id' => $new_cat->term_id, 'name' => $new_cat->name, 'parent' => $new_cat->parent);
        }
    ...
    public function fetch_source_html($url)
    {
        ...
        $allowed_elements = array('img' => array('src' => true, 'width' => true, 'height' => true), 'iframe' => array('src' => true), 'link' => array('rel' => true, 'itemprop' => true, 'href' => true), 'meta' => array('property' => true, 'name' => true, 'content' => true));
        ...
        if (preg_match('!/ad[sx]?/!i', $src)) {
            ...
            if (preg_match('!(/share-?this[^.]+?\\.[a-z0-9]{3,4})(\\?.*)?$!i', $src)) {
                ...
                if (preg_match('!/(spinner|loading|spacer|blank|rss)\\.(gif|jpg|png)!i', $src)) {
                    ...
                    if (preg_match('!/([^./]+[-_])?(spinner|loading|spacer|blank)s?([-_][^./]+)?\\.[a-z0-9]{3,4}!i', $src)) {
    ...
    public function source_data_fetch_fallback($url, $data = array())
    {
        ...
        if (preg_match_all('/<img [^>]+>/', $source_content, $matches)) {
            $items = $this->_limit_array($matches[0]);
            foreach ($items as $value) {
                if (preg_match('/width=(\'|")(\\d+)\\1/i', $value, $new_matches) && $new_matches[2] < 256 || preg_match('/height=(\'|")(\\d+)\\1/i', $value, $new_matches) && $new_matches[2] < 128) {
                    continue;
                }
 ...
 />
                ...
                foreach ($post_formats[0] as $format) {
                    ...
                    ?>
						<br />
						<input type="radio" name="post_format" class="post-format" id="post-format-<?php 
                    echo $attr_format;
                    ?>
" value="<?php 
                    echo $attr_format;
...
">
        ...
        if ($user_can_assign_terms) {
            ?>
			<button type="button" class="button-link tagcloud-link" id="link-post_tag"><?php 
    ...
    public function get_images($data)
    {
        ...
        if (!empty($data['_images'])) {
            foreach ($data['_images'] as $src) {
                ...
                $prot_relative_src = preg_replace('/^https?:/', '', $src);
                if (in_array($prot_relative_src, $this->images) || false !== strpos($src, 'avatar') && count($this->images) > 15) {
    ...
    public function get_suggested_content($data)
    {
        ...
        if (!empty($data['u']) && $this->_limit_embed($data['u'])) {
            ...
            $default_html['quote'] = '<blockquote>%1$s</blockquote>';
            $default_html['link'] = '<p>' . _x('Source:', 'Used in Press This to indicate where the content comes from.') . ' <em><a href="%1$s">%2$s</a></em></p>';
        }
        /**
