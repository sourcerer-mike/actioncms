...
class WP_Comments_List_Table extends WP_List_Table
{
    ...
    protected function get_views()
    {
        ...
        $stati = array('all' => _nx_noop('All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', 'comments'), 'moderated' => _nx_noop('Pending <span class="count">(%s)</span>', 'Pending <span class="count">(%s)</span>', 'comments'), 'approved' => _nx_noop('Approved <span class="count">(%s)</span>', 'Approved <span class="count">(%s)</span>', 'comments'), 'spam' => _nx_noop('Spam <span class="count">(%s)</span>', 'Spam <span class="count">(%s)</span>', 'comments'), 'trash' => _nx_noop('Trash <span class="count">(%s)</span>', 'Trash <span class="count">(%s)</span>', 'comments'));
        if (!EMPTY_TRASH_DAYS) {
            unset($stati['trash']);
        }
        ...
        foreach ($stati as $status => $label) {
            ...
            */
            $status_links[$status] = "<a href='{$link}'{$class}>" . sprintf(translate_nooped_plural($label, $num_comments->{$status}), sprintf('<span class="%s-count">%s</span>', 'moderated' === $status ? 'pending' : $status, number_format_i18n($num_comments->{$status}))) . '</a>';
        }
        /**
...
<?php 
        if ('top' === $which) {
            ?>
			<label class="screen-reader-text" for="filter-by-comment-type"><?php 
...
"><?php 
		...
		<input id="cb-select-<?php 
            echo $comment->comment_ID;
            ?>
" type="checkbox" name="delete_comments[]" value="<?php 
            echo $comment->comment_ID;
            ?>
" />
		<?php 
...
" class="hidden">
    ...
    public function column_date($comment)
    {
        ...
        $submitted = sprintf(__('%1$s at %2$s'), get_comment_date(__('Y/m/d'), $comment), get_comment_date(__('g:i a'), $comment));
