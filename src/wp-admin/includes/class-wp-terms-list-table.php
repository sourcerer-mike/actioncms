...
class WP_Terms_List_Table extends WP_List_Table
{
    ...
    public function column_cb($tag)
    {
        ...
        if (current_user_can(get_taxonomy($this->screen->taxonomy)->cap->delete_terms) && $tag->term_id != $default_term) {
            return '<label class="screen-reader-text" for="cb-select-' . $tag->term_id . '">' . sprintf(__('Select %s'), $tag->name) . '</label>' . '<input type="checkbox" name="delete_tags[]" value="' . $tag->term_id . '" id="cb-select-' . $tag->term_id . '" />';
        }
        return '&nbsp;';
    }
    /**
    ...
    public function column_name($tag)
    {
        ...
        $out = sprintf('<strong><a class="row-title" href="%s" aria-label="%s">%s</a></strong><br />', esc_url($edit_link), esc_attr(sprintf(__('&#8220;%s&#8221; (Edit)'), $tag->name)), $name);
...
" class="colspanchange">
	...
	<?php 
		...
		<p class="inline-edit-save submit">
			<button type="button" class="cancel button-secondary alignleft"><?php 
        _e('Cancel');
        ?>
</button>
			<button type="button" class="save button-primary alignright"><?php 
