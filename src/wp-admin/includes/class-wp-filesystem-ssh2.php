...
class WP_Filesystem_SSH2 extends WP_Filesystem_Base
{
    ...
    public function __construct($opt = '')
    {
        ...
        if (!function_exists('stream_get_contents')) {
            $this->errors->add('ssh2_php_requirement', sprintf(__('The ssh2 PHP extension is available, however, we require the PHP5 function %s'), '<code>stream_get_contents()</code>'));
            return;
        }
        // Set defaults:
        if (empty($opt['port'])) {
    ...
    public function connect()
    {
        ...
        if (!$this->link) {
            $this->errors->add('connect', sprintf(__('Failed to connect to SSH2 Server %s'), $this->options['hostname'] . ':' . $this->options['port']));
            return false;
        }
        if (!$this->keys) {
        ...
        if (!$this->sftp_link) {
            $this->errors->add('connect', sprintf(__('Failed to initialize a SFTP subsystem session with the SSH2 Server %s'), $this->options['hostname'] . ':' . $this->options['port']));
            return false;
        }
        return true;
    }
