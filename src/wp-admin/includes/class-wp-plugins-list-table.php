...
class WP_Plugins_List_Table extends WP_List_Table
{
    ...
    public function prepare_items()
    {
        ...
        wp_localize_script('updates', '_wpUpdatesItemCounts', array('plugins' => $js_plugins));
        if (!$orderby) {
            $orderby = 'Name';
        } else {
...
"/>
			<?php 
        submit_button($text, 'button hide-if-js', '', false, array('id' => 'search-submit'));
        ?>
		</p>
		<?php 
    }
    ...
    protected function extra_tablenav($which)
    {
        ...
        if ('recently_activated' == $status) {
            ...
            echo '<p>' . sprintf(__('Files in the %s directory are executed automatically.'), '<code>' . str_replace(ABSPATH, '/', WPMU_PLUGIN_DIR) . '</code>') . '</p>';
        } elseif ('top' === $which && 'dropins' === $status) {
            /* translators: %s: wp-content directory name */
            echo '<p>' . sprintf(__('Drop-ins are advanced plugins in the %s directory that replace WordPress functionality when present.'), '<code>' . str_replace(ABSPATH, '', WP_CONTENT_DIR) . '</code>') . '</p>';
        }
        echo '</div>';
    }
    /**
            ...
            switch ($column_name) {
                ...
                case 'description':
                    ...
                    if (isset($plugin_data['slug']) && current_user_can('install_plugins')) {
                        $plugin_meta[] = sprintf('<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>', esc_url(network_admin_url('plugin-install.php?tab=plugin-information&plugin=' . $plugin_data['slug'] . '&TB_iframe=true&width=600&height=550')), esc_attr(sprintf(__('More information about %s'), $plugin_name)), esc_attr($plugin_name), __('View details'));
